...
interface FeaturedCarouselProps {
  ...
  searching?: boolean;
  shorter?: boolean;
  forcedCategory?: "movies" | "tvshows" | "editorpicks" | "anime";
}
    ...
    const fetchFeaturedMedia = async () => {
      ...
      try {
        if (effectiveCategory === "movies" || effectiveCategory === "tvshows") {
          ...
          const animeData = await get<any>("/discover/tv", {
            api_key: conf().TMDB_READ_API_KEY,
            language: formattedLanguage,
            with_genres: "16",
          ...
          const animePromises = animeData.results
            .slice(0, FETCH_QUANTITY)
            .map((show: any) =>
              get<any>(`/tv/${show.id}`, {
                api_key: conf().TMDB_READ_API_KEY,
          ...
          const allAnime = animeDetails.map((show) => ({
            ...show,
            type: "show" as const,
          }));

          // Shuffle anime
          const shuffledAnime = [...allAnime].sort(() => 0.5 - Math.random());
          setMedia(shuffledAnime.slice(0, SLIDE_QUANTITY));
